...
actor {
  ...
  // New function to check if a username is available
  ...
  // Updated function to save user profile with unique username check
  ...
  public shared ({ caller }) func sendInvitation(id : Text, campaignId : Text, recipientUsername : Text, role : Role) : async () {
    ...
    switch (textMap.get(campaigns, campaignId)) {
      ...
      case (?campaign) {
        ...
        // Find recipient principal by username
  ...
  // Helper function to find principal by username
  ...
  // New function to allow participants to leave a campaign
  ...
  public shared ({ caller }) func createSession(id : Text, campaignId : Text, name : Text, description : Text, date : Text) : async () {
    ...
    switch (textMap.get(campaigns, campaignId)) {
      case (null) { Debug.trap("Campaign not found") };
      case (?_campaign) {
        // Check if caller is a GM for the campaign
        switch (campaignParticipantMap.get(campaignParticipants, campaignId)) {
          case (null) { Debug.trap("No participants found for this campaign") };
          case (?participants) {
  ...
  public shared ({ caller }) func addSessionPlayer(sessionId : Text, participant : Principal) : async () {
    ...
    switch (textMap.get(sessions, sessionId)) {
      ...
      case (?session) {
        ...
        // Check if participant is a player in the campaign
  ...
  public shared ({ caller }) func createNote(id : Text, sessionId : Text, name : Text, playerContent : Text, gmContent : Text, inGameDateTime : ?Text, tags : [Text]) : async () {
    ...
    switch (textMap.get(sessions, sessionId)) {
      ...
      case (?session) {
        ...
        // Check for unique note name within the campaign
        if (not isNoteNameUniqueInCampaign(session.campaignId, name, null)) {
  ...
  // Helper function to check if note name is unique within the campaign
  private func isNoteNameUniqueInCampaign(campaignId : Text, noteName : Text, currentNoteId : ?Text) : Bool {
    for ((noteId, note) in textMap.entries(notes)) {
      switch (textMap.get(sessions, note.sessionId)) {
        case (null) {};
        case (?session) {
          if (session.campaignId == campaignId and note.name == noteName) {
            switch (currentNoteId) {
  ...
  // New function to manage note access
  ...
  // New function to get note access for a participant
  ...
  // Helper function to check if caller has write access to a note
  ...
  // Updated function to allow note owner or users with write access to update a note
    ...
    switch (textMap.get(notes, id)) {
      ...
      case (?note) {
        ...
        switch (textMap.get(sessions, note.sessionId)) {
          case (null) { Debug.trap("Session not found") };
          case (?session) {
            if (not isNoteNameUniqueInCampaign(session.campaignId, name, ?id)) {
              Debug.trap("Note name must be unique within the campaign");
            };
          };
        };
  ...
  // Updated function to allow note owner or users with write access to delete a note
  ...
  public shared ({ caller }) func linkNotes(noteId : Text, targetNoteName : Text) : async () {
    ...
    switch (textMap.get(notes, noteId)) {
      ...
      case (?note) {
        ...
        // Find target note by name within the same campaign
        ...
        switch (targetNote) {
          ...
          case (?target) {
            ...
            if (linkExists(noteId, target.id)) {
              Debug.trap("A link already exists between these notes");
            };
  ...
  // Helper function to find note by name within the same campaign
  private func findNoteByNameInCampaign(sessionId : Text, noteName : Text) : ?Note {
    switch (textMap.get(sessions, sessionId)) {
      case (null) { null };
      case (?session) {
        // Find all notes in the same campaign
  ...
  // Helper function to add note link
  ...
  // Helper function to check if a link already exists between two notes
  private func linkExists(noteId : Text, linkedNoteId : Text) : Bool {
    switch (noteLinkMap.get(noteLinks, noteId)) {
      case (null) { false };
      case (?links) {
        for (link in links.vals()) {
          if (link.linkedNoteId == linkedNoteId) {
            return true;
          };
        };
        false;
  ...
  // Helper function to check if a participant has read access to a note
        ...
        switch (noteAccessMap.get(noteAccess, noteId)) {
          ...
          case (?accessList) {
            ...
            switch (access) {
              case (null) { false };
              case (?a) {
                // Check for Player Read (1) or GM Read (2) permissions
  ...
  // Function to get linked notes for a given note with details and read access info
  ...
  // Function to remove a note link
  public shared ({ caller }) func removeNoteLink(noteId : Text, linkedNoteId : Text) : async () {
    ...
    switch (textMap.get(notes, noteId)) {
      ...
      case (?note) {
        ...
        // Remove bidirectional link from linkedNoteId
  ...
  // New function to get all unique tags for a user's campaigns
  public query ({ caller }) func getUserCampaignTags() : async [Text] {
    ...
    // Find campaigns where the user is a participant
    for ((campaignId, participants) in campaignParticipantMap.entries(campaignParticipants)) {
      for (participant in participants.vals()) {
        if (participant.principal == caller) {
    ...
    // Collect unique tags from notes in user's campaigns
  ...
  // New function to get notes by tag for a user's campaigns
  public query ({ caller }) func getNotesByTag(tag : Text) : async [Note] {
    ...
    // Collect notes with the specified tag from user's campaigns
    for ((_, note) in textMap.entries(notes)) {
      switch (textMap.get(sessions, note.sessionId)) {
        case (null) {};
        case (?session) {
  ...
  public shared ({ caller }) func createMetadataNote(id : Text, name : Text, content : Text, linkedSessionNotes : [Text], campaignId : Text) : async () {
    ...
    // Check if the caller is the campaign owner or a participant
    switch (textMap.get(campaigns, campaignId)) {
      case (null) { Debug.trap("Campaign not found") };
      case (?campaign) {
        ...
        // Check for unique metadata note name within the campaign
  ...
  // Helper function to check if metadata note name is unique within the campaign
  ...
  // Helper function to check if caller is a participant in the campaign
  ...
  // Updated function to get metadata notes with access control
  ...
  // New function to get linked metadata notes for a session note
  ...
  // New function to unlink a metadata note from a session note
  ...
  // New function to get metadata note details with access control
  ...
  // New function to link a metadata note by name with campaign validation
  public shared ({ caller }) func linkMetadataNoteByName(sessionNoteId : Text, metadataNoteName : Text, campaignId : Text) : async () {
    ...
    switch (textMap.get(notes, sessionNoteId)) {
      ...
      case (?_sessionNote) {
        // Find the metadata note by name and campaignId
        ...
        switch (metadataNote) {
          ...
          case (?note) {
            // Add the session note ID to the linkedSessionNotes array
  ...
  // Helper function to find a metadata note by name and campaignId
